...
// --- Dynamic model list with 1h server-side cache ---
//
// Calls Anthropic's /v1/models endpoint, transforms the response into the
// shape the UI expects ({ value, label, description, shortLabel }), and
// caches it in memory for one hour. If the upstream call fails (no API key,
// network error, etc.), we serve a hardcoded fallback list so the UI is
...
let modelsCache = { models: null, fetchedAt: 0, source: "none" };
...
function shortLabelFromId(id) {
  ...
  // Convert "-4-5" tails into "-4.5" for nicer display.
  return stripped.replace(/-(\d+)-(\d+)$/, "-$1.$2");
}

function prettyLabelFromId(id) {
  const short = shortLabelFromId(id);
...
function transformAnthropicModels(rawList) {
  // Anthropic returns: { data: [{ id, display_name, created_at, type }, ...] }
  // We keep only Claude chat models (skip embeddings, etc. — current API only
  // returns chat models but be defensive) and sort newest-first by created_at.
  const items = (rawList || [])
  ...
  return items.map((m) => ({
    value: m.id,
    label: m.display_name || prettyLabelFromId(m.id),
    description: m.display_name ? `${m.display_name} (${m.id})` : m.id,
    shortLabel: shortLabelFromId(m.id),
  }));
}
...
async function fetchAnthropicModels() {
  ...
  if (!apiKey) {
    throw new Error("ANTHROPIC_API_KEY not set; cannot query /v1/models");
  }
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      throw new Error(`Anthropic /v1/models returned ${resp.status}: ${body.slice(0, 200)}`);
    }
    const json = await resp.json();
    const models = transformAnthropicModels(json.data);
    if (models.length === 0) {
      throw new Error("Anthropic returned zero claude- models");
    }
    return models;
  } finally {
...
async function getCachedModels({ forceRefresh = false } = {}) {
  const now = Date.now();
  if (
    !forceRefresh &&
    modelsCache.models &&
  ...
  try {
    ...
    modelsCache = { models, fetchedAt: now, source: "anthropic" };
    console.log(`[api] /api/models refreshed from Anthropic (${models.length} models)`);
  } catch (err) {
    console.warn(`[api] /api/models fetch failed, using fallback:`, err.message);
    // Only overwrite the cache if we don't already have a successful one,
    // so a transient failure doesn't poison a previously-good list.
    if (!modelsCache.models || modelsCache.source !== "anthropic") {
      modelsCache = { models: MODELS_FALLBACK, fetchedAt: now, source: "fallback" };
    }
  }
  return modelsCache;
}
...
app.get("/api/models", async (req, res) => {
  ...
  const cache = await getCachedModels({ forceRefresh: force });
  res.json({
    models: cache.models,
...
// Warm the cache on boot so the first UI request is instant.
